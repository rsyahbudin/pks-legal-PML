...
new #[Layout('components.layouts.app')] class extends Component {
    ...
    public function save(): void
    {
        ...
        // Handle file uploads (removed main document field)
        ...
        // Handle additional file uploads with organized structure
        $contractId = $contract->id;
        
        // Draft document
        if ($this->draft_document) {
            $draftPath = $this->draft_document->store("contracts/{$contractId}/draft", 'public');
            $contract->update(['draft_document_path' => $draftPath]);
        }
        
        // Mandatory documents (multiple files)
        if ($this->mandatory_documents && count($this->mandatory_documents) > 0) {
            $mandatoryPaths = [];
            foreach ($this->mandatory_documents as $index => $file) {
            ...
            $contract->update(['mandatory_documents_path' => $mandatoryPaths]);
        }
        
        // Approval document
        if ($this->approval_document) {
            $approvalPath = $this->approval_document->store("contracts/{$contractId}/approval", 'public');
            $contract->update(['approval_document_path' => $approvalPath]);
        }
